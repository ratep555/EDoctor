...
namespace Infrastructure.Data.Repositories
{
    public class PatientRepository : IPatientRepository
    {
        ...
        {   
            ...
            IEnumerable<int> ids = medicalrecords.Select(x => x.AppointmentId);

            var appointments = await _context.Appointments.Where(x => ids.Contains(x.Id)).ToListAsync();
        ...
        public async Task<int> GetCountForAllPatientsOfDoctor(int userId)
        {
            ...
            return await _context.Patients.Where(x => ids2.Contains(x.Id)).CountAsync();
        }
        ...
        public async Task<PatientStatisticsDto> ShowCountForEntitiesForPatient(int userId)
        {
            ...
            var medicalrecords = await _context.MedicalRecords.Include(x => x.Appointment)
                ...
                .ThenInclude(x => x.Office).Where(x => x.Appointment.Patient.ApplicationUserId == userId)
                .ToListAsync();
            
            IEnumerable<int> doctorids = medicalrecords.Select(x => x.Appointment.Office.DoctorId);
            IEnumerable<int> officeids = medicalrecords.Select(x => x.Appointment.OfficeId);
            ...
            patientstatistics.MedicalRecordsCount = await _context.MedicalRecords.Include(x => x.Appointment)
                .ThenInclude(x => x.Patient).Where(x => x.Appointment.Patient.ApplicationUserId == userId).CountAsync();
        ...
        public async Task<IEnumerable<PatientChartDto1>> GetNumberAndTypeOfMedicalRecordsForPatientForChart(int userId)
        {
            ...
            list.Add(new PatientChartDto1 { MedicalRecordType = "Hospital medical records", 
                ...
                .ThenInclude(x => x.Patient).Include(x => x.Appointment)
                .ThenInclude(x => x.Office).Where(x => x.Appointment.Office.HospitalId != null &&
                x.Appointment.Patient.ApplicationUserId == userId).CountAsync()  });
        ...
        public async Task<IEnumerable<PatientChartDto2>> GetNumberAndTypeOfOfficesForPatientForChart(int userId)
        {
            ...
            IEnumerable<int> hospitalofficesids = medicalrecords
                .Where(x => x.Appointment.Office.HospitalId != null).Select(x => x.Appointment.OfficeId);
